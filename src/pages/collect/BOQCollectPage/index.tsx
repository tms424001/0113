// src/pages/collect/BOQCollectPage/index.tsx
// 清单数据采集页面
...
import { getMockBOQFiles, getMockBOQCollectStats } from '@/mocks/boqCollect'
import { FileList } from './components/FileList'
import { RecordList } from './components/RecordList'
import { ImportFileModal } from './components/ImportFileModal'
import { PricingFileList } from './components/PricingFileList'
import styles from './BOQCollectPage.module.css'
 ...
 * 清单数据采集页面
 */
export const BOQCollectPage = () => {
  // 状态
  const [loading, setLoading] = useState(false)
  const [files, setFiles] = useState<BOQCollectFileDTO[]>([])
  const [selectedFile, setSelectedFile] = useState<BOQCollectFileDTO | null>(null)
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      // 加载统计
      const statsData = getMockBOQCollectStats()
      setStats(statsData)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          <Title level={4} style={{ margin: 0 }}>综价文件采集</Title>
          <Text type="secondary">
      ...
      <div className={styles.filterBar}>
        <Space wrap size="middle">
          ...
          />
          <Input placeholder="清单编码" style={{ width: 120 }} />
          <Space.Compact>
          ...
          <Select
            ...
            options={[
              { value: '', label: '全部' },
              ...BOQ_SOURCE_CHANNEL_OPTIONS.map((o) => ({
                value: o.value,
                label: o.label,
              })),
            ]}
